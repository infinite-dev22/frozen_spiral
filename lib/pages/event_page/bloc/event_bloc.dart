...
class EventBloc extends Bloc<EventEvent, EventState> {
  ...
  Future<FutureOr<void>> _mapGetActivitiesEventToState(
    ...
    await EventApi.fetchAll(event.body).then((events) {
  ...
  Future<FutureOr<void>> _mapGetEventEventToState(
    ...
    await EventApi.fetch(event.eventId).then((event) {
      emit(state.copyWith(status: EventStatus.eventSuccess, event: event));
    }).onError((error, stackTrace) {
      if (kDebugMode) {
        print(error);
        print(stackTrace);
  ...
  Future<FutureOr<void>> _mapPostEventEventToState(
    ...
    emit(state.copyWith(status: EventStatus.eventsLoading));
    await EventApi.post(event.event, event.eventId).then((event) {
      emit(state.copyWith(status: EventStatus.eventSuccess, event: event));
    }).onError((error, stackTrace) {
      if (kDebugMode) {
        print(error);
  ...
  Future<FutureOr<void>> _mapPutEventEventToState(
    ...
    emit(state.copyWith(status: EventStatus.eventsLoading));
    await EventApi.put(event.event, event.eventId).then((event) {
      emit(state.copyWith(status: EventStatus.eventSuccess, event: event));
    }).onError((error, stackTrace) {
      if (kDebugMode) {
        print(error);
